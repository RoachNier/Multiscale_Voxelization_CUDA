...
__global__ void assign_point_to_voxel(const int nthreads, const T* points,
                                      ...
                                      const int num_points, const int NDim, const int scale_idx, const int max_voxels) {
  CUDA_1D_KERNEL_LOOP(thread_idx, nthreads) {
    ...
    if (num > -1 && voxelidx > -1) {
      auto voxels_offset =
          voxels + voxelidx * max_points * num_features + num * num_features + scale_idx * max_voxels * max_points * num_features;
...
__global__ void assign_voxel_coors(const int nthreads, T_int* coor,
  ...
  CUDA_1D_KERNEL_LOOP(thread_idx, nthreads) {
    ...
    int voxelidx = coor_to_voxelidx[index];
    if (num == 0 && voxelidx > -1) {
      auto coors_offset = voxel_coors + voxelidx * NDim + scale_idx * max_voxels * NDim;
...
__global__ void point_to_voxelidx_kernel(const T_int* coor,
  ...
  CUDA_1D_KERNEL_LOOP(index, num_points) {
    ...
    // skip invalid points // 如果当前线程超过点云数目或者当前线程指向的点云在范围之外，退出线程
...
__global__ void determin_voxel_num(
    ...
    const int max_points, const int max_voxels, const int num_points, const int scale_idx) {
  // only calculate the coors before this coor[index]
  auto num_points_per_voxel_scale = num_points_per_voxel + scale_idx * max_voxels;
  for (int i = 0; i < num_points; ++i) {
    ...
    if (point_pos_in_voxel == -1) {
      ...
      num_points_per_voxel_scale[voxelidx] = 1;
    } else {
...
std::vector<int> hard_voxelize_gpu(const at::Tensor& points, at::Tensor& voxels,
  ...
  const int scale_num = max_pointses.size();
  const int max_points_for_all_scale = *max_element(max_pointses.begin(), max_pointses.end());
  ...
  std::vector<int> voxel_nums;

  dim3 cp_grid(std::min(at::cuda::ATenCeilDiv(pts_output_size, 512), 4096));
  dim3 cp_block(BLOCKDIM);

  dim3 coors_cp_grid(
      std::min(at::cuda::ATenCeilDiv(coors_output_size, 512), 4096));
  dim3 coors_cp_block(BLOCKDIM);

  for (int i = 0; i < scale_num; ++i) {
    
    const int max_points = max_pointses[i];
    auto voxel_size = voxel_sizes[i];
  ...
  // create some temporary variables
    point_to_pointidx.fill_(-1);
    point_to_voxelidx.fill_(-1);
  ...
  // Step 4 & 5 could be parallel
        ...
        points.scalar_type(), "assign_point_to_voxel", ([&] {
          assign_point_to_voxel<float, int>
              <<<cp_grid, cp_block, 0, at::cuda::getCurrentCUDAStream()>>>(
                  ...
                  point_to_voxelidx.contiguous().data_ptr<int>(),
                  coor_to_voxelidx.contiguous().data_ptr<int>(),
                  voxels.contiguous().data_ptr<float>(), max_points_for_all_scale, num_features,
                  num_points, NDim, i, max_voxels); // 多传一个scale idx，也就是i
        }));
    //   cudaDeviceSynchronize();
    //   AT_CUDA_CHECK(cudaGetLastError());
    ...
    AT_DISPATCH_ALL_TYPES(
        points.scalar_type(), "assign_point_to_voxel", ([&] {
          assign_voxel_coors<float, int><<<coors_cp_grid, coors_cp_block, 0,
              ...
              point_to_voxelidx.contiguous().data_ptr<int>(),
              coor_to_voxelidx.contiguous().data_ptr<int>(),
              coors.contiguous().data_ptr<int>(), num_points, NDim, i, max_voxels);
        }));
    cudaDeviceSynchronize();
    AT_CUDA_CHECK(cudaGetLastError());
    ...
    voxel_nums.push_back(voxel_num_int);
  }
  return voxel_nums;
}
